...
def get_dict(enc_type):
    res = {}
    dd = pathlib.Path(__file__).parent
    with open(str(dd.joinpath("data/%s.json")) % enc_type, 'r') as load_f:
        encoding = json.load(load_f)
    res[enc_type] = encoding
    return res
...
class SequenceEncoding:
    ...
    def __init__(self, encoding_type="One_hot", enc_dict=None):
    ...
    def get_encoding(self, seq, overlap=True):
        seq = seq.upper()
        encoding = self.enc_dict[self.encoding_type]
        encoding_data = []
...
def main():
    ...
    parser.add_argument('--overlap', dest='overlap', action='store_true',
                        help="The word extraction strategy for ProtVec encoding: overlap")
    parser.add_argument('--non-overlap', dest='overlap', action='store_false',
                        help="The word extraction strategy for ProtVec encoding: non-overlap")
